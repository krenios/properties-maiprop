...
import { Monitor, ArrowUpRight, Sparkles, MapPin, Bed, Square, Heart, Search, BarChart2, Gavel, Home, TrendingUp, Layers, Bell } from "lucide-react";
...
// ─── Live flicker hook ────────────────────────────────────────────────────────
const CITIES = [
  { city: "Athens",       base: 2840, change: 1.2,  up: true,  spark: "M0,14 C4,10 8,18 12,12 C16,6 20,16 24,10 C28,4 32,14 36,8 C40,2 44,12 48,8" },
  { city: "Thessaloniki", base: 1920, change: 0.8,  up: true,  spark: "M0,12 C4,16 8,8 12,14 C16,20 20,10 24,16 C28,12 32,18 36,12 C40,8 44,14 48,10" },
  { city: "Mykonos",      base: 6450, change: 2.3,  up: true,  spark: "M0,16 C4,8 8,14 12,6 C16,2 20,10 24,4 C28,8 32,4 36,10 C40,6 44,14 48,8" },
  { city: "Glyfada",      base: 3760, change: 1.7,  up: true,  spark: "M0,10 C4,14 8,8 12,12 C16,16 20,8 24,14 C28,10 32,6 36,12 C40,8 44,12 48,6" },
  { city: "Rhodes",       base: 2299, change: 3.2,  up: true,  spark: "M0,8 C4,12 8,4 12,10 C16,6 20,14 24,8 C28,4 32,10 36,6 C40,12 44,8 48,4" },
  { city: "Santorini",    base: 5120, change: 1.9,  up: true,  spark: "M0,14 C4,10 8,16 12,8 C16,4 20,12 24,6 C28,10 32,4 36,8 C40,2 44,10 48,6" },
  { city: "Kifisia",      base: 4180, change: 0.6,  up: true,  spark: "M0,12 C4,8 8,14 12,10 C16,6 20,12 24,8 C28,14 32,10 36,14 C40,8 44,12 48,10" },
];

function useLiveFlicker() {
  const [prices, setPrices] = useState<Record<string, number>>(
    Object.fromEntries(CITIES.map(c => [c.city, c.base]))
  );
...
// ─── Listing-site mockup ──────────────────────────────────────────────────────
const MOCK_CARDS = [
  { title: "Kolonaki Apartment", city: "Athens", beds: 2, sqm: 85, price: "€420,000", badge: "New", img: "https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?w=400&q=70" },
  { title: "Mykonos Villa", city: "Mykonos", beds: 4, sqm: 220, price: "€1,850,000", badge: "Golden Visa", img: "https://images.unsplash.com/photo-1582719508461-905c673771fd?w=400&q=70" },
  { title: "Glyfada Penthouse", city: "Glyfada", beds: 3, sqm: 140, price: "€680,000", badge: "Featured", img: "https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=400&q=70" },
];
  ...
  return (
    <div className="w-full bg-[hsl(222_47%_7%)] text-white overflow-hidden" style={{ minHeight: 360 }}>
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/8 bg-[hsl(222_47%_6%)]">
        <span className="font-black text-sm tracking-tight text-white">mAI <span style={{ color: "hsl(192 100% 50%)" }}>Prop</span></span>
      ...
      <div className="flex items-center gap-0 overflow-hidden border-b border-white/6 bg-[hsl(222_47%_5%)] px-0 py-0">
        <div className="flex items-center gap-1.5 px-3 py-1.5 border-r border-white/8 shrink-0">
          <span className="relative flex w-1.5 h-1.5">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full opacity-60" style={{ background: "hsl(192 100% 50%)" }} />
            <span className="relative inline-flex rounded-full w-1.5 h-1.5" style={{ background: "hsl(192 100% 50%)" }} />
          </span>
          <span className="text-[9px] font-black tracking-widest uppercase" style={{ color: "hsl(192 100% 60%)" }}>Live</span>
        </div>
        <div className="flex gap-4 px-3 py-1.5 overflow-hidden">
          {CITIES.slice(0, 5).map(c => (
            <span key={c.city} className="flex items-center gap-1.5 shrink-0 text-[10px]">
              ...
              <span className="font-bold tabular-nums" style={{ color: "hsl(192 100% 70%)" }}>€{c.base.toLocaleString()}</span>
              <span className="text-[9px]" style={{ color: c.up ? "hsl(142 72% 55%)" : "hsl(0 72% 55%)" }}>{c.up ? "▲" : "▼"}{c.change}%</span>
            </span>
          ))}
        </div>
      </div>
      ...
      <div className="relative px-4 pt-4 pb-2">
        <div className="text-[11px] font-black tracking-widest uppercase mb-1" style={{ color: "hsl(192 100% 50%)" }}>Greece's #1 AI Property Portal</div>
        <div className="text-base font-black leading-tight mb-3">Find Your Perfect Greek<br /><span style={{ color: "hsl(192 100% 55%)" }}>Golden Visa Property</span></div>
        {/* Stats bar */}
        <div className="flex gap-3 mb-4">
          {[["12,400+", "Listings"], ["3,200+", "Auctions/mo"], ["€250k", "Min GV"]].map(([v, l]) => (
            <div key={l} className="flex-1 rounded-lg bg-white/5 border border-white/8 px-2 py-1.5 text-center">
              <div className="text-xs font-black" style={{ color: "hsl(192 100% 60%)" }}>{v}</div>
      ...
      <div className="relative px-4 pb-3 overflow-hidden" style={{ maxHeight: 170 }}>
            ...
            <div key={i} className="flex gap-3 mb-2.5 rounded-xl bg-white/5 border border-white/8 overflow-hidden">
              ...
              <div className="flex-1 py-2 pr-3 min-w-0">
                <div className="flex items-center gap-1.5 mb-0.5">
                  <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full" style={{ background: "hsl(192 100% 50% / 0.15)", color: "hsl(192 100% 60%)" }}>{card.badge}</span>
                </div>
                ...
                <div className="flex items-center justify-between mt-1">
                  <div className="flex items-center gap-2 text-[9px] text-white/50">
                    <span className="flex items-center gap-0.5"><Bed className="w-2.5 h-2.5" />{card.beds}</span>
                    <span className="flex items-center gap-0.5"><Square className="w-2.5 h-2.5" />{card.sqm}m²</span>
                  </div>
                  <span className="text-[11px] font-black" style={{ color: "hsl(192 100% 60%)" }}>{card.price}</span>
                </div>
              </div>
            </div>
          ))}
        ...
        {/* Fade out */}
        <div className="absolute inset-x-0 bottom-0 h-10 pointer-events-none" style={{ background: "linear-gradient(to bottom, transparent, hsl(222 47% 7%))" }} />
      </div>
    </div>
  );
}
...
function OSDashboardMockup() {
  ...
  const chartH = 52;
  const ptStr = CHART_POINTS.map((v, i) => `${(i / (CHART_POINTS.length - 1)) * 100},${chartH - (v / maxPts) * chartH}`).join(" ");
  ...
  return (
    <div className="w-full flex bg-[hsl(225_35%_6%)] text-white overflow-hidden" style={{ minHeight: 360 }}>
      ...
      <div className="w-10 shrink-0 flex flex-col items-center gap-3 py-3 border-r border-white/8 bg-[hsl(225_35%_5%)]">
        <div className="w-6 h-6 rounded-lg flex items-center justify-center" style={{ background: "hsl(263 86% 64% / 0.2)", border: "1px solid hsl(263 86% 64% / 0.4)" }}>
          <span className="text-[8px] font-black" style={{ color: "hsl(263 86% 75%)" }}>OS</span>
        </div>
        ...
        {[BarChart2, MapPin, Gavel, Home, Layers].map((Icon, i) => (
          <button key={i} className="w-7 h-7 rounded-lg flex items-center justify-center transition-colors"
            style={{ background: i === 0 ? "hsl(263 86% 64% / 0.2)" : "transparent", border: i === 0 ? "1px solid hsl(263 86% 64% / 0.3)" : "1px solid transparent" }}>
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-white/8 bg-[hsl(225_35%_5%)]">
          ...
          <div className="flex items-center gap-1.5 px-2 py-1 rounded-full text-[9px] font-black"
            style={{ background: "hsl(263 86% 64% / 0.15)", border: "1px solid hsl(263 86% 64% / 0.3)", color: "hsl(263 86% 75%)" }}>
        ...
        <div className="flex flex-1 gap-0 overflow-hidden">
          ...
          <div className="w-1/2 border-r border-white/8 overflow-hidden">
            ...
            <div className="px-3 pt-2.5 pb-1">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[10px] font-black" style={{ color: "hsl(263 86% 75%)" }}>+4.2% YoY</span>
              </div>
              <svg width="100%" height={chartH + 4} viewBox={`0 0 100 ${chartH + 4}`} preserveAspectRatio="none" className="overflow-visible">
                <defs>
                ...
                <polygon
                  points={`0,${chartH + 4} ${ptStr} 100,${chartH + 4}`}
                  fill="url(#osGrad)"
                />
                <polyline
                  points={ptStr}
            ...
            <div className="divide-y" style={{ borderColor: "hsl(263 86% 64% / 0.07)" }}>
              {CITIES.slice(0, 5).map((row) => {
                ...
                return (
                  <div key={row.city} className="flex items-center gap-2 px-3 py-1.5 transition-all duration-300"
                    style={{ background: isFlick ? "hsl(263 86% 64% / 0.08)" : "transparent" }}>
                    <span className="flex-1 text-[10px] transition-colors duration-300" style={{ color: isFlick ? "hsl(263 86% 80%)" : "hsl(215 20% 60%)" }}>{row.city}</span>
                    <span className="text-[10px] font-bold tabular-nums transition-colors duration-300" style={{ color: isFlick ? "hsl(263 86% 75%)" : "hsl(210 40% 90%)" }}>
                      €{livePrice.toLocaleString()}
                    </span>
                    <span className="text-[9px] font-black w-10 text-right" style={{ color: row.up ? "hsl(142 72% 55%)" : "hsl(0 72% 55%)" }}>
                      {row.up ? "▲" : "▼"}{row.change}%
                    </span>
                  </div>
                );
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            ...
            <div className="px-3 pt-2 pb-1">
              <div className="text-[9px] font-bold text-white/30 uppercase tracking-widest mb-1.5">AI Investment Score</div>
              <div className="space-y-1.5">
                {REGIONS.map((r, i) => (
                  <div key={r.name} className="transition-all duration-500"
                    style={{ opacity: i === activeRegion ? 1 : 0.55, transform: i === activeRegion ? "scale(1.01)" : "scale(1)" }}>
                    <div className="flex items-center justify-between mb-0.5">
                      <span className="text-[9px]" style={{ color: i === activeRegion ? "hsl(263 86% 80%)" : "hsl(215 20% 55%)" }}>{r.name}</span>
                      <span className="text-[9px] font-black" style={{ color: i === activeRegion ? "hsl(263 86% 75%)" : "hsl(215 20% 45%)" }}>{r.score}</span>
                    </div>
                    <div className="h-1 rounded-full bg-white/8 overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-700"
                        style={{
                          width: `${r.score}%`,
                          background: i === activeRegion
            ...
            <div className="mt-auto px-3 py-2 border-t border-white/8" style={{ background: "hsl(270 40% 8% / 0.6)" }}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[9px] font-black tracking-wider uppercase" style={{ color: "hsl(270 75% 75%)" }}>Judicial Auctions</span>
                </div>
                <span className="text-[10px] font-black px-1.5 py-0.5 rounded-md" style={{ color: "hsl(270 75% 75%)", background: "hsl(270 75% 62% / 0.15)" }}>3,241</span>
              </div>
              <div className="flex gap-1.5 mt-1.5">
                {[["Athens", "€95k"], ["Thessaloniki", "€48k"], ["Patras", "€32k"]].map(([city, bid]) => (
                  <div key={city} className="flex-1 rounded-md px-2 py-1 text-center" style={{ background: "hsl(270 40% 12% / 0.8)", border: "1px solid hsl(270 75% 62% / 0.2)" }}>
                    <div className="text-[8px] text-white/40 truncate">{city}</div>
                    <div className="text-[9px] font-black" style={{ color: "hsl(270 75% 70%)" }}>{bid}</div>
                  </div>
                ))}
              </div>
            </div>
...
const platforms = [
  {
    ...
    desc: "The most intelligent real estate listing portal in Greece — live market data, AI-powered search, auctions, open houses and Golden Visa properties, all in one place.",
    bullets: [
      "Mark to Market Valuation of every listing",
      "Access to Auctions & Open Houses",
      "AI property search & side-by-side analytics",
      "Golden Visa compliant properties filtered",
    ],
              ...
              <ScrollReveal>
                <div className={`grid items-center gap-12 lg:gap-20 lg:grid-cols-2 py-16 sm:py-24 ${p.reverse ? "lg:[direction:rtl]" : ""}`}>
                  ...
                  <div className={p.reverse ? "[direction:ltr]" : ""}>
                    ...
                    <Button asChild size="lg" className={`gap-2 rounded-full px-8 font-semibold transition-all ${p.ctaStyle}`}>
                  ...
                  <div className={`relative ${p.reverse ? "[direction:ltr]" : ""}`}>
                    <div className={`pointer-events-none absolute -inset-6 rounded-3xl blur-[60px] opacity-20 ${p.glowClass}`} />
